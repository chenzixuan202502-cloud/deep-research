...
def get_retriever_tool(resources: List[Resource]) -> RetrieverTool | None:
    ...
    # Build retriever based on the provided resources
    # This will only initialize providers that match the resource URIs
    retriever = build_retriever(resources)

    if not retriever:
        logger.warning("No retriever could be built for the given resources")
        return None
