...
async def researcher_node(
    ...
    configurable = Configuration.from_runnable_config(config)
    logger.info(f"[researcher_node] Max search results: {configurable.max_search_results}")   ##############debug
    # logger.info(f"Full Configuration: {configurable}")  ####################### 为查看 max_search_results 用

    tools = [get_web_search_tool(configurable.max_search_results), crawl_tool]
    retriever_tool = get_retriever_tool(state.get("resources", []))
    if retriever_tool:
        logger.info(f"[researcher_node] Adding retriever tool to tools list")##############debug
        tools.insert(0, retriever_tool)
    
    logger.info(f"[researcher_node] Researcher tools count: {len(tools)}")
    logger.info(f"[researcher_node] Researcher tools: {[tool.name if hasattr(tool, 'name') else str(tool) for tool in tools]}")##############debug
