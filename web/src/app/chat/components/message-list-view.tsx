...
function extractThinkBlocks(content: string) {
  ...
  // Only extract the FIRST <think>...</think> block as thinking, the rest stays in main content
  const firstThinkMatch = content.match(/<think>([\s\S]*?)<\/think>/);
  if (!firstThinkMatch) {
    return { main: content, think: "" };
  }
  const thinkContent = firstThinkMatch[1]?.trim() ?? "";
  // Everything after the first </think> becomes the main content
  const mainContent = content.slice(firstThinkMatch[0].length).trim();
  return { main: mainContent, think: thinkContent };
}
...
function MessageListItem({
  ...
  const showThinking = useSettingsStore((state) => state.general.showThinking);
  const startOfResearch = useMemo(() => {
    return researchIds.includes(messageId);
  }, [researchIds, messageId]);
  if (message) {
    const { main: mainContentFromTags, think: thinkFromTags } = useMemo(() => {
      // If backend already provides reasoningContent, don't re-extract from <think>.
      if (message.reasoningContent && message.reasoningContent.trim() !== "") {
        return { main: message.content, think: "" };
      }
      return extractThinkBlocks(message.content);
    ...
    const hasMainContent = Boolean(
      mainContentFromTags && mainContentFromTags.trim() !== "",
    );
    ...
    if (
      ...
      if (message.agent === "planner") {
        ...
        const renderContent = message.role === "assistant" ? mainContentFromTags : message.content;
      ...
      if (content) {
        return (
          ...
          >
            {message.role === "assistant" && reasoningContent && showThinking && (
              <ThoughtBlock
