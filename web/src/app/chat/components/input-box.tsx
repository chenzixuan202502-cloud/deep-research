...
import { ArrowUp, Globe, Lightbulb, X } from "lucide-react";
...
import { SearchApiDialog } from "~/components/deer-flow/search-api-dialog";
...
export function InputBox({
  ...
  const normalAnswer = useSettingsStore(
    (state) => state.general.enableNormalAnswer,
  );
  const webSearch = useSettingsStore(
    (state) => state.general.enableWebSearch,
  );
  const searchProvider = useSettingsStore(
        ...
        <div className="flex grow items-center gap-2">
          ...
          <Tooltip
            ...
            title={
              <div>
                <h3 className="mb-2 font-bold">
                  {t("normalAnswerTooltip.title", {
                    status: normalAnswer ? t("on") : t("off"),
                  })}
                </h3>
                <p>{t("normalAnswerTooltip.description")}</p>
              </div>
            }
          >
            <Button
              className={cn(
                "rounded-2xl",
                normalAnswer && "!border-brand !text-brand",
              )}
              variant="outline"
              onClick={() =>
                setEnableNormalAnswer(!normalAnswer)
              }
            >
              <Lightbulb /> {t("normalAnswer")}
            </Button>
          </Tooltip>
          <Tooltip
            className="max-w-60"
          ...
          >
            <Button
              className={cn(
                "rounded-2xl",
                webSearch && "!border-brand !text-brand",
              )}
              variant="outline"
              onClick={() =>
