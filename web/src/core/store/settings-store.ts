...
const DEFAULT_SETTINGS: SettingsState = {
  general: {
    ...
    enableNormalAnswer: false,
    enableWebSearch: false,
    maxPlanIterations: 1,
    maxStepNum: 3,
    maxSearchResults: 3,
    reportStyle: "academic",
    searchProvider: "infoquest",
    showThinking: true,
  },
  mcp: {
    servers: [],
...
export function setEnableNormalAnswer(value: boolean) {
  useSettingsStore.setState((state) => ({
    general: {
      ...state.general,
      enableNormalAnswer: value,
    },
  }));
  saveSettings();
}
...
export function setSearchProvider(value: "tavily" | "infoquest") {
  useSettingsStore.setState((state) => ({
    general: {
      ...state.general,
