...
/**
 * Maps frontend locale to backend format
 * @param frontendLocale - The locale from next-intl (e.g., "zh", "en")
 * @returns Backend locale format (e.g., "zh-CN", "en-US")
 */
export function mapLocaleToBackend(frontendLocale: string): string {
  return LOCALE_MAP[frontendLocale as keyof typeof LOCALE_MAP] ?? "en-US";
}
...
export async function* chatStream(
  ...
  params: {
    thread_id: string;
    locale: string; // Now required from caller
    user_id?: string;
    chat_seq?: number;
    ...
    search_provider?: "tavily" | "infoquest";
    ...
    answer_mode?: "normal" | "deep_research";
  },
