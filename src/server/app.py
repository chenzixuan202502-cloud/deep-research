...
from src.server.auth import auth_router
from src.server.history import history_router, save_chat_message_to_db, create_new_chat_session
...
async def chat_stream(request: ChatRequest):
    ...
    logger.info(f"[CHAT_STREAM] Received request: answer_mode={request.answer_mode}, user_id={request.user_id}, chat_seq={request.chat_seq}")
    ...
    # Normal chat mode: directly call LLM without research workflow
    if request.answer_mode == "normal":
        logger.info(f"[CHAT_STREAM] Using normal chat mode, enable_web_search={request.enable_web_search}")
        return StreamingResponse(
            _astream_normal_chat_generator(request, request.enable_web_search),
            media_type="text/event-stream",
        )

    # Deep research mode: use existing workflow
    thread_id = request.thread_id
    if thread_id == "__default__":
        thread_id = str(uuid4())

    # 获取 user_id 和 chat_seq 用于保存历史
...
async def _astream_normal_chat_generator(request: ChatRequest, enable_web_search: bool = False):
    """
    Generator for normal chat mode - directly calls LLM without research workflow.
    """
    import openai
    from src.config import load_yaml_config
    import os

    logger.info(f"_astream_normal_chat_generator called with messages: {request.messages}, enable_web_search={enable_web_search}")
    ...
    logger.info(f"user_id: {user_id}, chat_seq: {chat_seq}, enable_web_search={enable_web_search}")

    # 如果没有 chat_seq，则创建一个新会话（只有当 user_id 有效时）
    if not chat_seq and user_id and user_id.strip():
        new_chat_result = create_new_chat_session(user_id)
        if new_chat_result:
            chat_seq = new_chat_result
            logger.info(f"Created new chat session: {chat_seq}")

    # 获取用户的问题（最后一条 user 消息）
    question = ""
    ...
    if not openai_messages:
        yield f"event: error\ndata: {json.dumps({'error': 'No valid messages'})}\n\n"
        return

    # Load model config from conf.yaml
    conf_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "conf.yaml")
    config = load_yaml_config(conf_path)
    basic_model = config.get("BASIC_MODEL", {})

    # 构建带历史的消息列表
    from src.server.history import get_chat_history
    history_messages = get_chat_history(chat_seq) if chat_seq else []

    # 拼接历史消息
    openai_messages_with_history = []
    for q, a in history_messages:
        if q:  # 添加用户问题
            openai_messages_with_history.append({"role": "user", "content": q})
        if a:  # 添加助手回答
            openai_messages_with_history.append({"role": "assistant", "content": a})
    # 添加当前消息
    openai_messages_with_history.extend(openai_messages)

    logger.info(f"[NORMAL_CHAT] History messages count: {len(history_messages)}, "
                f"Total messages for LLM: {len(openai_messages_with_history)}")
    ...
    if enable_web_search:
        logger.info(f"[NORMAL_CHAT] Using web search mode")
        try:
            # 导入联网检索处理器
            from src.tools.web_search_for_normal_chat import NormalChatWebSearcher
            web_searcher = NormalChatWebSearcher()

            # 1. 意图识别 - 提取关键词
            keywords = await web_searcher._extract_keywords(question, history_messages)
            logger.info(f"[NORMAL_CHAT] Extracted keywords: {keywords}")

            # 发送关键词提取完成状态
            yield f"event: message\ndata: {json.dumps({'status': 'keywords_extracted', 'keywords': keywords})}\n\n"

            # 2. 联网搜索
            search_results = await web_searcher._web_search(keywords)
            logger.info(f"[NORMAL_CHAT] Web search completed, results length: {len(search_results)}")

            # 发送搜索完成状态
            yield f"event: message\ndata: {json.dumps({'status': 'search_completed', 'results_length': len(search_results)})}\n\n"

            # 提取搜索结果中的标题和链接，用于后续附在答案后面
            reference_sources = web_searcher.extract_reference_sources(search_results)
            logger.info(f"[NORMAL_CHAT] Extracted {len(reference_sources)} reference sources")
...
5. 回答完成后，在答案末尾附上参考来源（标题和链接）"""

            user_prompt = f"""历史对话:
{chr(10).join([f"用户: {q}\n助手: {a}" for q, a in history_messages])}
...
请根据以上信息回答用户的问题:"""
            ...
            response = client.chat.completions.create(
                model=basic_model.get("model", "glm-4.7-flash"),
                messages=[
            ...
            for chunk in response:
                if chunk.choices and len(chunk.choices) > 0:
                    ...
                    if delta.content:
                        ...
                        event_data = {'status': 'streaming', 'chunk': delta.content}
            ...
            if reference_sources:
                ...
                for i, source in enumerate(reference_sources, 1):
                    reference_text += f"{i}. [{source['title']}]({source['url']})\n"
                full_answer += reference_text

            # 发送完成状态
            completed_data = {'status': 'completed', 'full_content': full_answer}
            yield f"event: message\ndata: {json.dumps(completed_data, ensure_ascii=False)}\n\n"

            # 保存问答到数据库
            if user_id and chat_seq and question and full_answer:
                ...
                logger.info(f"[NORMAL_CHAT] Background save task submitted for chat_seq={chat_seq}")

        except Exception as e:
            logger.error(f"Error in normal chat with web search: {e}")
        ...
        try:
            ...
            full_answer = ""
            for chunk in response:
                #logger.info(f"Normal chat chunk: {chunk}")
                if chunk.choices and len(chunk.choices) > 0:
                    delta = chunk.choices[0].delta
                    if delta.content:
                        #logger.info(f"Normal chat content: {delta.content}")
                        full_answer += delta.content
                        event_data = {'status': 'streaming', 'chunk': delta.content}
                        #logger.info(f"[NORMAL_CHAT] [FRONTEND_DATA] streaming: {json.dumps(event_data, ensure_ascii=False)}")
                        yield f"event: message\ndata: {json.dumps(event_data)}\n\n"

            # 发送完成状态
            completed_data = {'status': 'completed', 'full_content': full_answer}
            #logger.info(f"[NORMAL_CHAT] [FRONTEND_DATA] completed: {json.dumps(completed_data, ensure_ascii=False)[:2000]}")
            yield f"event: message\ndata: {json.dumps(completed_data)}\n\n"

            # 保存问答到数据库（使用线程池执行，不阻塞响应流）
            if user_id and chat_seq and question and full_answer:
                import concurrent.futures
                executor = concurrent.futures.ThreadPoolExecutor(max_workers=1)
                executor.submit(
                ...
                logger.info(f"Background save task submitted for chat_seq={chat_seq}")
...
@app.post("/api/deep-research/stream")         #接入模型所新建的接口
async def deep_research_stream(request: ChatRequest):
    """
    深度研究接口 - 预设深度检索的默认参数
    
    与 /api/chat/stream 的区别：
    - 默认 auto_accepted_plan=True（自动接受计划，不中断）
    - 默认 max_plan_iterations=2（最多2轮计划迭代）
    - 默认 max_step_num=3（最多3个执行步骤）
    - 默认 max_search_results=3（每次搜索最多3条结果）
    - 默认 enable_background_investigation=True（启用背景调研）
    - 默认 report_style=academic（学术风格报告）
    ...
    logger.info(f"Deep research stream started with locale: {request.locale}")
    ...
    # 设置预设默认值（如果外部系统没传）
    # 这些是深度研究推荐的默认值
    # 注意：ChatRequest 中 auto_accepted_plan 默认是 False，所以用 False 判断
    if request.auto_accepted_plan is None or request.auto_accepted_plan is False:
        request.auto_accepted_plan = True
    
    if request.max_plan_iterations is None:
        request.max_plan_iterations = 2
    
    if request.max_step_num is None:
        request.max_step_num = 3
    
    if request.max_search_results is None:
        request.max_search_results = 3
    
    if request.enable_background_investigation is None:
        request.enable_background_investigation = True
    
    if request.report_style is None:
    ...
    logger.info(
        f"Deep research config: auto_accepted_plan={request.auto_accepted_plan}, "
        f"max_plan_iterations={request.max_plan_iterations}, "
        f"max_step_num={request.max_step_num}"
    )
...
async def _astream_workflow_generator(
    ...
    if not auto_accepted_plan and interrupt_feedback:
        ...
        # Use Command with update to preserve locale and other state
    ...
    # 流结束后保存问答到数据库（暂时禁用，排查前端显示问题）
    # if user_id and chat_seq and messages:
    #     try:
    #         # 获取用户的问题（最后一条用户消息）
    #         user_question = ""
    #         for msg in reversed(messages):
    ...
    #                 break
    #
    #         # 获取 AI 的回答（从 thread_id 的存储中获取，或者通过其他方式）
    #         # 这里简化处理：如果有完整的对话内容，可以保存
    #         # 实际实现可能需要更复杂的逻辑来收集 AI 回复
    #         if user_question:
    #             logger.info(f"[{safe_thread_id}] Saving chat to history: user_id={user_id}, chat_seq={chat_seq}")
    #             # 调用保存接口
    #             from src.server.history import save_chat_message
    #             # 这里使用空的 answer，因为流式响应结束时我们可能没有完整的回答
    #             # 实际实现中，可能需要通过其他方式获取完整回答
    #             # 暂时不保存空的 answer
    #             # await save_chat_message(user_id, chat_seq, user_question, "")
    #     except Exception as e:
    #         logger.error(f"[{safe_thread_id}] Error saving chat history: {e}")
