...
class ChatRequest(BaseModel):
    user_id: Optional[str] = Field(
        None, description="User ID for history tracking"
    )
    chat_seq: Optional[int] = Field(
        None, description="Chat sequence number for history tracking"
    )
    ...
    search_provider: Optional[str] = Field(
        default=None,
        description="Preferred web search provider for this request (e.g., 'tavily', 'infoquest')",
    )
    answer_mode: Optional[str] = Field(
        "deep_research",
        description="Answer mode: 'normal' for simple chat, 'deep_research' for research workflow",
    )
    enable_web_search: Optional[bool] = Field(
        False,
        description="Whether to enable web search for normal chat mode",
    )
    model: Optional[str] = Field(
        "Qwen3-30B-abliterated",
        description="Model name to use for normal chat mode",
    )
