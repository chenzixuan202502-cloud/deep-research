...
def normalize_locale(locale: str | None) -> str:
    """
    Normalize locale to workflow-supported values: zh / en
    """
    if not locale:
        return "zh"
    locale = locale.lower()
...
async def run_agent_workflow_async(
    ...
    if initial_state is None:
        ...
        print("初次深度检索主题为:", initial_state["research_topic"])
    ...
    last_message_cnt = 0
    final_state = None
    graph_circle = 0    #这个是调试用的，我看看为什么进不去深度检索
    async for s in graph.astream(
        input=initial_state, config=config, stream_mode="values"
    ):
        graph_circle += 1   #调试用的，这个变量没啥用
        try:
            final_state = s
            print(f"当前消息总数: {len(s.get('messages', []))}")
            print("第", graph_circle, "圈图执行结果:", s)
    ...
    if final_state and isinstance(final_state, dict):
        ...
        if needs_clarification(final_state):
            ...
            print("澄清轮数为:", clarification_rounds)
