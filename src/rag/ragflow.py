...
class RAGFlowProvider(Retriever):
    ...
    def __init__(self):
        ...
        if not api_url:
            api_url = os.getenv("RAGFLOW_BASE_URL")
        if not api_url:
    ...
    def query_relevant_documents(
        ...
        if not dataset_ids:
            default_dataset_ids = os.getenv("RAGFLOW_DATASET_IDS")
            if not default_dataset_ids:
                default_dataset_ids = os.getenv("RAGFLOW_DEFAULT_DATASET_IDS")
            if not default_dataset_ids:
                default_dataset_ids = os.getenv("RAGFLOW_KB_ID")
            if default_dataset_ids:
                dataset_ids = [s.strip() for s in default_dataset_ids.split(",") if s.strip()]
            else:
                try:
                    dataset_ids = [parse_uri(r.uri)[0] for r in self.list_resources()]
                except Exception:
                    dataset_ids = []
