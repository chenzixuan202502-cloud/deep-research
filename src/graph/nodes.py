...
def validate_and_fix_plan(plan: dict, enforce_web_search: bool = False) -> dict:
    ...
    return plan         #进行强制的联网检索


def background_investigation_node(state: State, config: RunnableConfig):      #背景检索
    ...
    if SELECTED_SEARCH_ENGINE == SearchEngine.TAVILY.value:
        ...
        # Handle legacy list format
            ...
            ## logger.info(f"info debug: Background_investigator: background_investigation_results={background_investigation_results}")
            return {
                "background_investigation_results": "\n\n".join(
                    background_investigation_results
                )
...
def planner_node(
    ...
    ## logger.info(f"info debug: planner: plan_iterations={plan_iterations}")
    ...
    if plan_iterations >= configurable.max_plan_iterations:
        ## logger.info(f"info debug: planner: iterations is greater than the max plan iterations, return the reporter node")
        return Command(
            update=preserve_state_meta_fields(state),
            goto="reporter"
        )

    ## logger.info(f"info debug: planner: messages={messages}")
    full_response = ""
    ...
    ## logger.info(f"info debug: planner: curr_plan={curr_plan}")
    if isinstance(curr_plan, dict) and curr_plan.get("has_enough_context"):
        logger.info("Planner response has enough context.")
        new_plan = Plan.model_validate(curr_plan)
        ## logger.info(f"info debug: planner: new_plan={new_plan}")
        return Command(
            update={
...
def human_feedback_node(
    ...
    ## logger.info(f"info debug: human_feedback_node: current_plan={update_dict["current_plan"]}")
    ## logger.info(f"info debug: human_feedback_node: goto={goto}")
    return Command(
        update=update_dict,
        goto=goto,
    )
...
def coordinator_node(
    ...
    ## logger.info(f"info debug: Coordinator: enable_clarification={enable_clarification}")
    # ============================================================
    ...
    if not enable_clarification:
        ...
        ## logger.info(f"info debug: Coordinator: LLM response={response}")
        goto = "__end__"
        locale = state.get("locale", "zh-CN")
            ...
            if clarification_rounds >= max_clarification_rounds:
                ...
                return Command(
                    update={
                        ...
                        "resources": existing_resources,  # Use parsed resources
    ...
    ## logger.info(f"info debug: Coordinator: goto={goto}, clarified_research_topic_value={clarified_research_topic_value}\n, messages={messages}")
    ...
    return Command(
        update={
            ...
            "resources": existing_resources,  # Use parsed resources instead of configurable.resources
            "clarification_rounds": clarification_rounds,
            "clarification_history": clarification_history,
...
def reporter_node(state: State, config: RunnableConfig):
    ...
    ## logger.info(f"info debug: [reporter] locale={state.get("locale", "zh-CN")}")
    invoke_messages = apply_prompt_template("reporter", input_, configurable, input_.get("locale", "zh-CN"))
    ## logger.info(f"info debug: [reporter] invoke_messages={invoke_messages}")
...
async def _execute_agent_step(
    ...
    ## logger.info(f"info debug: [_execute_agent_step] Starting execution for agent: {agent_name}")
    ...
    ## logger.info(f"info debug: [{agent_name}] {observations + [response_content]}")
    return Command(
        update={
        ...
        goto="research_team",
    )           #非常关键的函数，是确保·各个节点在传递指令的时候格式确保一致的关键
...
async def researcher_node(
    ...
    ## logger.info(f"info debug: [researcher_node] Researcher tools: {[tool.name if hasattr(tool, 'name') else str(tool) for tool in tools]}")
