...
def parse_database_tags(text: str) -> list[Resource]:
    """
    ...
    Args:
        text: User input text that may contain database tags
        
    Returns:
        List of Resource objects corresponding to the detected tags
    """
    resources = []
    ...
    if re.search(elasticsearch_pattern, text, re.IGNORECASE):
        ...
        logger.info("Detected @Elasticsearch tag, added Elasticsearch resource")
    
    # Check for RAG tags
    if re.search(rag_pattern, text, re.IGNORECASE):
        # For RAGFlow, we need to use a generic dataset URI
        # In a real scenario, you might want to list available datasets
        resources.append(Resource(
            uri="rag://dataset/default",
            title="RAGFlow 知识库",
            description="RAGFlow向量数据库知识库"
        ))
        logger.info("Detected @RAG tag, added RAGFlow resource")
    
    return resources


def extract_user_message_content(state: State) -> str:
    """
    Extract the content of the last user message from state.
    ...
    return state.get("research_topic", "")


def simple_post_process_plan(full_response: str) -> str:
    """
    Simple post-process the plan to ensure it meets requirements.
    """
    while full_response[0] == '`' and full_response[-1] == '`':
        full_response = full_response[1:-1]
    while full_response[0] != '{' and full_response[0] != '[':
...
def planner_node(
    ...
    logger.info("Planner generating full plan with locale: %s", state.get("locale", "zh-CN"))  ################## zh-CN
    configurable = Configuration.from_runnable_config(config)
    ...
    if state.get("enable_clarification", False) and state.get(
        ...
        messages = apply_prompt_template("planner", modified_state, configurable, state.get("locale", "zh-CN"))  ####################zh-CN
        ...
        messages = apply_prompt_template("planner", state, configurable, state.get("locale", "zh-CN")) #####################zh-CN
    ...
    print('enable_deep_thinking: ', configurable.enable_deep_thinking)  ##############
    ...
    # Debug: Print actual messages content before invoking LLM
    logger.warning(f"[DEBUG] ========== PLANNER INPUT MESSAGES START ==========")
    for i, msg in enumerate(messages):
        content = msg.content if hasattr(msg, 'content') else str(msg)
        role = msg.type if hasattr(msg, 'type') else type(msg).__name__
        logger.warning(f"[DEBUG] Message {i} [{role}]:\n{content[:5000]}")  # Print first 5000 chars per message
        if len(content) > 5000:
            logger.warning(f"[DEBUG] ... (truncated, total {len(content)} chars)")
    logger.warning(f"[DEBUG] ========== PLANNER INPUT MESSAGES END ==========")
    ...
    while attempt < MAX_PLAN_ATTEMPTS:
        full_response = ""
        attempt += 1
        logger.info(f"Planner attempt {attempt}/{MAX_PLAN_ATTEMPTS}")
        ...
        # ============================================================
        # DEBUG: 记录原始响应内容（用于诊断 JSON 格式问题）
        # ============================================================
        logger.info(f"Planner raw response (first 500 chars): {full_response[:500]}")
        logger.info(f"Planner raw response length: {len(full_response)}")
        
        full_response = simple_post_process_plan(full_response)  ################
        print(f"After a simple post process, the Planner response: {full_response}")  ##############

        # ============================================================
        # DEBUG: 记录后处理后的响应内容
        # ============================================================
        logger.info(f"Planner post-processed response (first 500 chars): {full_response[:500]}")
        ...
        starts_with_brace = stripped_response.startswith('{')
        starts_with_bracket = stripped_response.startswith('[')
        
        logger.info(f"JSON format check - starts with '{{': {starts_with_brace}, starts with '[': {starts_with_bracket}")
        
        if not starts_with_brace and not starts_with_bracket:
            logger.warning(f"[JSON DIAGNOSIS] Response does not start with '{{' or '['. First 100 chars: {stripped_response[:100]}")
            continue
        else:
    ...
    if parsed_response is None:
        logger.warning(f"Planner response does not appear to be valid JSON after {MAX_PLAN_ATTEMPTS} attempts")
    ...
    try:
        # ============================================================
        # DEBUG: 记录 repair_json_output 之前的内容
        # ============================================================
        logger.info(f"Before repair_json_output (first 500 chars): {full_response[:500]}")
        
        repaired_response = repair_json_output(full_response)
        logger.info(f"After repair_json_output (first 500 chars): {repaired_response[:500]}")
        
        curr_plan = json.loads(repaired_response)
        logger.info(f"First json.loads succeeded, plan type: {type(curr_plan)}")
        
        # Need to extract the plan from the full_response
        curr_plan_content = extract_plan_content(curr_plan)
        
        # ============================================================
        # DEBUG: 记录提取后的 plan 内容
        # ============================================================
        logger.info(f"Extracted plan content (first 500 chars): {curr_plan_content[:500]}")
        
        # load the current_plan
        curr_plan = json.loads(repair_json_output(curr_plan_content))
        logger.info(f"Second json.loads succeeded, plan type: {type(curr_plan)}")
        
    except json.JSONDecodeError as e:
        # ============================================================
        # DEBUG: 详细的 JSON 解析错误信息
        # ============================================================
        logger.warning(f"[JSON DIAGNOSIS] Planner response is not a valid JSON: {e}")
        logger.warning(f"[JSON DIAGNOSIS] Error position: {e.pos}, lineno: {e.lineno}, colno: {e.colno}")
        logger.warning(f"[JSON DIAGNOSIS] Full response for debugging:\n{full_response}")
        logger.warning(f"[JSON DIAGNOSIS] Response length: {len(full_response)}")
        ...
        try:
            ...
            if e.pos and e.pos > 0:
                ...
                error_context_end = min(len(full_response), e.pos + 50)
                error_context = full_response[error_context_start:error_context_end]
                logger.warning(f"[JSON DIAGNOSIS] Error context around position {e.pos}: ...{error_context}...")
        except Exception:
            pass
        ...
        # DEBUG: 其他异常信息
        # ============================================================
        logger.warning(f"[JSON DIAGNOSIS] Unexpected error during JSON parsing: {type(e).__name__}: {e}")
        logger.warning(f"[JSON DIAGNOSES] Full response for debugging:\n{full_response}")
...
def human_feedback_node(
    ...
    try:
        ...
        # Handle case where new_plan is a list (incorrect format from planner)
        if isinstance(new_plan, list):
            logger.warning(f"Plan parsed as list: {new_plan}")
            # Handle ['', {...}] format - extract dict from second element
            if len(new_plan) == 2 and isinstance(new_plan[1], dict):
                new_plan = new_plan[1]
                logger.info("Extracted plan dict from list[1]")
            # Try to find the dict in the list
            else:
                for item in new_plan:
                    ...
                    # If no dict found, use first element if it's not empty
                    if new_plan and len(new_plan) > 0:
                        first_item = new_plan[0]
                        if isinstance(first_item, dict):
                            new_plan = first_item
                        else:
                            logger.error(f"Cannot extract valid plan dict from: {new_plan}")
                            return Command(
                                update=preserve_state_meta_fields(state),
                                goto="__end__"
                            )
    ...
    # Build update dict - locale is preserved via preserve_state_meta_fields()
    # NOTE: Locale SSOT - locale is managed exclusively in State, never from Plan
    # The preserve_state_meta_fields() function already includes locale preservation
    
    # Ensure new_plan is a dict before validation
    if not isinstance(new_plan, dict):
        logger.error(f"new_plan is not a dict after validation: {type(new_plan).__name__}, {new_plan}")
        return Command(
            update=preserve_state_meta_fields(state),
            goto="__end__"
        )

    # Check if the dict is actually a tool call structure instead of a real Plan
    # (LLM sometimes returns handoff/tool calls instead of plan JSON)
    if "tool" in new_plan and "title" not in new_plan:
        logger.warning(
            f"new_plan looks like a tool call, not a Plan object: {str(new_plan)[:200]}. "
            f"Re-routing to planner."
        )
        return Command(
            update={
                "messages": [
                    HumanMessage(
                        content="Please output the research plan in the required JSON format.",
                        name="feedback",
                    )
                ],
                **preserve_state_meta_fields(state),
    ...
    required_fields = {"title", "has_enough_context"}
    missing_fields = required_fields - new_plan.keys()
    if missing_fields:
        logger.warning(
            f"new_plan is missing required fields {missing_fields}: {str(new_plan)[:200]}. "
            f"Re-routing to planner."
        )
        return Command(
            update={
                "messages": [
                    HumanMessage(
                        content="Please output the research plan in the required JSON format with all required fields.",
                        name="feedback",
                    )
                ],
                **preserve_state_meta_fields(state),
    ...
    # REMOVED: Locale override logic - Plan should never override State locale
    # This prevents LLM hallucination from corrupting the authoritative locale value
...
def coordinator_node(
    ...
    # 1. 获取语言 locale
    print('locale in state = ', state['locale'])  ###################
    locale = state.get("locale", "zh-CN")
    print('In coordinator node, initial locale = ', locale)   ##################
   
    # ============================================================
    # TAG PARSER: Parse database tags from user input
    # ============================================================
    # Extract the last user message content
    user_message_content = extract_user_message_content(state)
    
    # Parse database tags and convert to Resource objects
    parsed_resources = parse_database_tags(user_message_content)
    
    # Merge with existing resources (avoid duplicates by URI)
    existing_resources = list(state.get("resources", []) or [])
    existing_uris = {r.uri for r in existing_resources}
    
    for resource in parsed_resources:
        if resource.uri not in existing_uris:
            existing_resources.append(resource)
            logger.info(f"Added parsed resource: {resource.title} ({resource.uri})")
    
    # Update state with merged resources
    if parsed_resources:
        logger.info(f"Total resources after tag parsing: {len(existing_resources)}")
    
    # ============================================================
    # INJECT VIRTUAL GLOBAL RESOURCE
    # ============================================================
    # If no resources exist after parsing, inject a virtual "Global Knowledge Base"
    # This ensures Planner always considers internal database search
    if not existing_resources:
        ...
        logger.info("Injected virtual 'Global Knowledge Base' resource into state.")
    else:
        # Even if resources exist, ensure global resource is present
        ...
        if not has_global:
            ...
            logger.info("Added virtual 'Global Knowledge Base' resource to existing resources.")
    ...
    if not enable_clarification:
        ...
        print('In coordinator node, before apply template, locale = ', locale)   ##################
        
        # 2. 构建 System Prompt 添加语言约束
        messages = apply_prompt_template("coordinator", state, locale=locale)
        messages.append(
            {
                "role": "system",
                "content": f"IMPORTANT: You must output your plan and response in {locale} language.",
            }
        )
        messages.append(
            {
        ...
        print('In coordinator node, after invoke messages, locale = ', locale)  #################
        research_topic = state.get("research_topic", "")
    ...
    return Command(
        update={
            ...
            "resources": existing_resources,  # Use updated resources with virtual global resource
            "clarification_rounds": clarification_rounds,
            "clarification_history": clarification_history,
...
def reporter_node(state: State, config: RunnableConfig):
    ...
    configurable = Configuration.from_runnable_config(config)
    current_plan = state.get("current_plan")
    print('In reporter_node, locale = ', state.get('locale'))   ##############################
    input_ = {
        "messages": [
            HumanMessage(
        ...
        "locale": state.get("locale", "zh-CN"), #####################zh-CN
    }
...
async def _execute_agent_step(
    ...
    plan_title = current_plan.title if current_plan else "No Title"  # --- 新增日志代码 ---
    plan_steps_count = len(current_plan.steps) if current_plan and hasattr(current_plan, 'steps') else 0   # --- 新增日志代码 ---
    
    logger.info(f"[_execute_agent_step] Agent: {agent_name} | Executing Plan: {plan_title} | Steps: {plan_steps_count}")   # 1. 写入结构化日志（用于保存到文件）
    ...
    try:
        ...
        logger.info(f"parsed_limit is: {parsed_limit}")
    ...
    logger.info(f"should_validate: {response_content}")
    validation_info = ""
    ...
    logger.info(f"Tool_message_count: {tool_message_count}")
...
async def researcher_node(
    ...
    """Researcher node that do research with hybrid search"""
    logger.info("Researcher node is researching.")
    logger.debug(f"[researcher_node] Starting researcher agent")
    
    configurable = Configuration.from_runnable_config(config)
    logger.info(f"[researcher_node] Max search results: {configurable.max_search_results}")
    
    # ============================================================
    # HYBRID SEARCH INTEGRATION - DISABLED (Virtual Resource Injection Active)
    # ============================================================
    # REASON: Agent now intelligently calls local_search_tool based on Plan.
    # Forced parallel_search causes unnecessary performance overhead and Token waste.
    # Virtual Resource Injection ensures Agent has access to retrieval tools.
    # ============================================================
    
    # from src.graph.hybrid_search import parallel_search
    
    # # Extract query from current step
    # current_plan = state.get("current_plan")
    # current_step = None
    # if current_plan:
    ...
    # # Execute hybrid search if we have a step to research
    # hybrid_context = ""
    # if current_step:
    #     query = f"{current_step.title} {current_step.description}"
    #     logger.info(f"[Hybrid Search] Executing parallel search for: {query}")
    #     
    #     try:
    #         # Execute parallel search across Web, RAG, and ES
    ...
    #         
    #         if hybrid_context:
    #             logger.info(f"[Hybrid Search] Successfully retrieved context: {len(hybrid_context)} chars")
    #         else:
    #             logger.warning("[Hybrid Search] No results from parallel search")
    #             
    #     except Exception as e:
    #         logger.error(f"[Hybrid Search] Failed to execute parallel search: {e}")
    #         import traceback
    #         traceback.print_exc()
    
    # # ============================================================
    # # INJECT HYBRID CONTEXT INTO STATE
    # # ============================================================
    # if hybrid_context:
...
# Please use this context to inform your research, but also feel free to search for additional information if needed.""",
    #         name="hybrid_search_context"
    #     )
    #     
    #     # Add context to state messages
    #     current_messages = list(state.get("messages", []))
    #     current_messages.append(context_message)
    #     
    #     # Update state with injected context
    #     state = {**state, "messages": current_messages}
    #     logger.info("[Hybrid Search] Context injected into state messages")
    
    # ============================================================
    # CONTINUE WITH NORMAL RESEARCHER FLOW
    # ============================================================
    tools = [
        get_web_search_tool(
            configurable.max_search_results,
            getattr(configurable, "search_provider", None),
        ),
        crawl_tool,
    ]
    retriever_tool = get_retriever_tool(state.get("resources", []))
    if retriever_tool:
        logger.info(f"[researcher_node] Adding retriever tool to tools list")
        tools.insert(0, retriever_tool)
    
    logger.info(f"[researcher_node] Researcher tools count: {len(tools)}")
    logger.info(f"[researcher_node] Researcher tools: {[tool.name if hasattr(tool, 'name') else str(tool) for tool in tools]}")
