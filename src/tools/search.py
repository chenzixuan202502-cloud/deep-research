...
def _resolve_selected_engines(override: Optional[str] = None) -> list[str]:
    """
    Resolve the list of selected engines, allowing an optional override.

    If override is provided (e.g. \"tavily\" or \"infoquest\"), it takes precedence.
    Otherwise fall back to the SELECTED_SEARCH_ENGINE environment variable
    which may contain a comma-separated list.
    """
    raw_value = override or SELECTED_SEARCH_ENGINE
    return [engine.strip() for engine in raw_value.split(",") if engine.strip()]
...
def get_web_search_tool(
    ...
    selected_engines = _resolve_selected_engines(search_provider)
    print(f"SELECTED_SEARCH_ENGINE: {SELECTED_SEARCH_ENGINE}, override={search_provider}")
    for engine in selected_engines:
        if engine == SearchEngine.TAVILY.value:
            ...
            include_raw_content: bool = search_config.get("include_raw_content", False)       #修改了，让其不返回原网页的内容
        ...
        elif engine == SearchEngine.INFOQUEST.value:
        ...
        elif engine == SearchEngine.SERPER.value:
        ...
        elif engine == SearchEngine.SEARX.value:
        ...
        elif engine == SearchEngine.WIKIPEDIA.value:
